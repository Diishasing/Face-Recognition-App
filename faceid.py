...
class CamApp(App):
    ...
    def verify(self, *args):
        ...
        SAVE_PATH = os.path.join('app_data', 'input_images', 'input_images .jpg')
        ret, frame = self.capture.read()
        frame = frame[120:120+250, 200:200+250, :]
        ...
        for image in os.listdir(os.path.join('app_data', 'verification_image')):
            input_img = self.preprocess(os.path.join('app_data', 'input_images', 'input_images.jpg'))
            validation_img = self.preprocess(os.path.join('app_data', 'verification_image', image))
